...
def create_new_ids(force = 0):
    ...
    if os.path.isfile('../new_ids_v2.csv') and force == 0:
        ...
        ids.to_csv('../new_ids_v2.csv', index = False)
        print ('\n\tFile new_ids_v2.csv created successfully!\n\t')
...
def append_transactions(prefix = 'merged_', force = 0):
    ...
    """
    if os.path.isfile('../' + prefix + 'transactions.csv') and force == 0:
            print ('File %s already exists. Nothing created' %(prefix+'transactions.csv'))
            pass
    else:
        print ('merging transaction files...')
        txn_og = pd.read_csv('../transactions.csv')
...
def save_files_new_ids(files = ['members_v2.csv', 'train_v2.csv', 'merged_transactions.csv', 'sample_submission_v2.csv'], prefix = 'new_', force = 0, force_userlog = 0):
    """
    ...
    """
    new_ids = pd.read_csv('../new_ids_v2.csv')
    list_files = files
...
def transactions_merged():
    """
    Read and parse the info for the merged txn file (smallint, dates, ...)
    It already uses the optimum data types to reduce memory load
...
def train_v2():
    """
    Read and parse the info for the new train file (smallint, dates, ...)
    It already uses the optimum data types to reduce memory load

    :return: new train DataFrame parsed with correct data types
    """
    train_file = pd.read_csv('../new_train_v2.csv', dtype={
            'is_churn': np.int8,
            'new_id': np.uint32})
